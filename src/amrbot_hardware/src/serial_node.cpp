...
struct tx_payload
{
    ...
    uint8_t motor_start;
    uint8_t enc_l_reversed;
    uint8_t enc_r_reversed;
...
{
    RCLCPP_INFO(this->get_logger(), "r,l,r,g,yaw: %d ; %d ; %d ; %d ; %.2f", rx_data.driver_ready, 
    rx_data.enc_l_dt, rx_data.enc_r_dt, rx_data.gpio, rx_data.yaw_deg);
...
void serial_node::tx_callback()
{
    ...
    tx_data.enc_l_reversed = 0;
    tx_data.enc_r_reversed = 1;
    tx_data.motor_l_speed = 5;
    tx_data.motor_r_speed = 5;

    memcpy(tx_buffer + 3, &tx_data.driver_start, 1);
    memcpy(tx_buffer + 4, &tx_data.motor_start, 1);
    memcpy(tx_buffer + 5, &tx_data.enc_l_reversed, 1);
    memcpy(tx_buffer + 6, &tx_data.enc_r_reversed, 1);
    memcpy(tx_buffer + 7, &tx_data.motor_l_speed, 2);
    memcpy(tx_buffer + 9, &tx_data.motor_r_speed, 2);
